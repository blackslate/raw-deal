;(function createImageMatchPuzzle(lx){
  ...
  ImageMatch.prototype.updateScore = function updateScore(result, showProgress) {
    var correct
      , worst
  ...
  ImageMatch.prototype.initialize = function initialize() {
    ...
    var p = document.querySelector(".cards p")
    var answerImgs = [].slice.call(answers.querySelectorAll("img"))
    var path = "img/images/"
    var remaining = 2
    var distractors = []
    var imageLUT = {}
    ...
    function newCard(){
      cardface.classList.remove("fadeIn")     
      p.classList.remove("fadeIn")     
  
      card = lx.randomItemFromArray(that.cues, that.repeatAfter)
      overlay.src = imageLUT[card].src
      p.innerHTML = imageMap[card]
      ...
      function chooseDistractors() {
        ...
        for (ii = 0; ii < total; ii += 1) {
          ...
          img.src = lx.createCard({card: key}).src
        }        
      }
    }
    ...
    function answer(event){
      ...
      var index = answerImgs.indexOf(target)
     
      if (distractors[index] === card) {
        cardface.src = target.src
